...
<html lang="en" dir="ltr">
  ...
  <body>
    <?php 
        ...
        $sql_department = "SELECT * FROM tb_department WHERE id_department = $id_department";
        $result_department = mysqli_query($con,$sql_department);
        $row = mysqli_fetch_assoc($result_department);
    ?>
    <div class="sidebar">
            ...
            <div class="right-side">
                ...
                <form action="update_department.php" method="post">
                    <div class="form-group">
                        <label for="usr">ชื่อแผนก</label>
                        <input type="text" class="form-control" id="usr" name="d_name" value="<?php echo $row['d_name'] ;?>">
                    </div>
                    <button type="submit" class="btn btn-primary"><i class='bx bx-save' ></i> บันทึก</button>
                    <a href="data_department.php" class="btn btn-danger"><i class='bx bx-x' ></i> ยกเลิก</a>
                    <input type="hidden" name="id_department" value="<?php echo $_GET['id'];?>">
                </form>
            </div>
          </div>
        </div>
