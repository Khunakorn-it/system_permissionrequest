  ...
  <body>
    ...
    <?php 
        ...
        function percent($con,$d){
          $sql_information = "SELECT COUNT(id_request) as sum_request FROM tb_request_join";
          ...
          if($all_number != 0){
            if($d != "อื่นๆ"){
              $sql_data = "SELECT COUNT(id_request) as sum_request FROM tb_request_join WHERE reason = '$d'";
              $result_data = mysqli_query($con,$sql_data);
              $row_data = mysqli_fetch_assoc($result_data);
              $all_data = $row_data['sum_request'];
              
            }else{
              $sql_data = "SELECT COUNT(id_request) as sum_request FROM tb_request_join WHERE reason != 'ออกไปรับประทานอาหารด้านนอก' AND reason != 'ออกไปปริ้นงาน' AND reason != 'ลืมหนังสือ/สมุด'";
              $result_data = mysqli_query($con,$sql_data);
              $row_data = mysqli_fetch_assoc($result_data);
              $all_data = $row_data['sum_request'];
            }
    ...
    <section class="home-section">
      <nav>
        <div class="sidebar-button">
          ...
          <span class="dashboard">รายงานสถิติรวม</span>
        </div>
      </nav>
        ...
        <div class="overview-boxes">
          <div class="box">
            <div class="right-side">
              <div class="box-topic">จำนวนการขออนุญาตทั้งหมด</div>
    ...
    <script type="text/javascript">
                ...
                title: {
                    text: 'รายงานจำนวนการขออนุญาตออกนอกวิทยาลัยของนักเรียนนักศึกษา',
                    align: 'center'
                },
                yAxis: {
                    title: {
